...
import { EcopilotNav } from "@/components/ecopilot/EcopilotNav";
 ...
 * into this app's (dashboard)/ecopilot route. Keeps its own internal
 * navigation (profile/season/language switcher + tabs) exactly as
 * originally designed; it just now renders inside the site's existing
 * Header/Sidebar chrome instead of as a standalone page.
 */
export function EcopilotApp() {
  ...
  return (
    <div className="rounded-2xl border bg-slate-50 text-slate-800 overflow-hidden -m-4 md:-m-6">
      <EcopilotNav
      ...
      <footer className="border-t border-slate-200 bg-white py-6 px-4 text-xs text-slate-500">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 bg-emerald-600 rounded flex items-center justify-center text-white font-bold text-[10px]">E</div>
