...
import { ArrowLeft, Map as MapIcon, MapPin, Plus, Save, X } from "lucide-react";
...
import { FavouritePlacesMap, type FavouritePlaceMarker } from "@/components/map/FavouritePlacesMap";
...
export function FrequentPlacesView({
  ...
  // Home doesn't change on this page, so it's read straight from the profile;
  // place pins come from live `places` state so the map updates as an address
  // is looked up, without waiting for Save.
  const homeMarker = hasHomeAddress ? { lat: userProfile.homeLat as number, lon: userProfile.homeLon as number } : null;
  const placeMarkers = useMemo<FavouritePlaceMarker[]>(
    () =>
      places
        .filter((p): p is FrequentPlace & { lat: number; lon: number } => p.lat != null && p.lon != null)
        .map((p) => ({
          id: p.id,
          lat: p.lat,
          lon: p.lon,
          label: p.label.trim() || (isFinnish ? "Nimetön paikka" : "Unnamed place"),
          address: p.address,
          icon: p.icon,
        })),
    [places, isFinnish]
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 pb-8 space-y-6 animate-fadeIn">
        <ViewHero
          accent="indigo"
          isFinnish={isFinnish}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px] gap-6 items-start">
                  ...
                  <div key={place.id} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5 space-y-2.5">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <input
                        ...
                        className="visible-text-cursor flex-1 min-w-0 px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-emerald-500"
                      />
                      <select
          ...
          <div className="rounded-3xl border border-slate-200 shadow-sm bg-white overflow-hidden lg:sticky lg:top-6">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-2">
              <h3 className="text-xs font-black text-slate-800 uppercase tracking-wide flex items-center gap-1.5">
                ...
                {isFinnish ? "Kartta" : "Map"}
              </h3>
              <span className="text-[12px] font-bold text-slate-400 shrink-0">
                {placeMarkers.length + (homeMarker ? 1 : 0)} {isFinnish ? "nastaa" : "pins"}
              </span>
            </div>
            <div className="relative h-[320px] lg:h-[480px]">
              <FavouritePlacesMap home={homeMarker} places={placeMarkers} isFinnish={isFinnish} />
              {placeMarkers.length === 0 && !homeMarker && (
                <div className="absolute inset-x-3 bottom-3 rounded-xl bg-white/95 border border-slate-200 shadow-sm px-3 py-2 text-[12px] text-slate-500 font-medium pointer-events-none">
                  {isFinnish
                    ? "Ei nastoja vielä — lisää kotiosoite profiilissa tai paikka alla."
                    : "No pins yet — add a home address in your profile or a location below."}
                </div>
              )}
            </div>
          </div>
