...
export function NordPoolEnergyOptimizerView({ userProfile, currentSeason, outdoorTempCelsius, isFinnish }: NordPoolEnergyOptimizerViewProps) {
  ...
  const peakCo2Grams = kiuasKwh * 135;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-500">
                {isFinnish
                  ? "Vihreä = Erittäin halpa & puhdas tuuli, Punainen = Iltahuippu"
                  : "Green = Cheap & clean wind, Red = Peak demand window"}
              </p>
            </div>
              ...
              {MOCK_HOURLY_SPOT_PRICES.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span className="text-[9px] text-slate-400 font-mono mt-1 hidden sm:block">{p.hour}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-emerald-50 border border-amber-200/80 space-y-3">
              ...
              <p className="text-[11px] text-amber-950 italic leading-relaxed">
                ...
                {aiEnergyPlan?.peakSaunaWindow.reason ||
                  (isFinnish
                    ? "Klo 21:00 jälkeen kantaverkon sähkö on 70% halvempaa ja huomattavasti puhtaampaa."
                    : "After 21:00, grid electricity is ~70% cheaper and significantly cleaner with strong wind output.")}
                &quot;
              </p>
            </div>
          </div>
