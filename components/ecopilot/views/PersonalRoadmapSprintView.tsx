...
import { Sparkles, CheckCircle2, Circle, RotateCw, Building, Heart, Share2 } from "lucide-react";
import type { UserProfile, Season } from "@/lib/ecopilot/types";
import { generateRoadmapPlanAPI, type RoadmapPlan } from "@/lib/ecopilot/client";
...
export function PersonalRoadmapSprintView({ userProfile, currentSeason, isFinnish, onOpenShareModal }: PersonalRoadmapSprintViewProps) {
  const [isGenerating, setIsGenerating] = useState<boolean>(false);
  const [sprintData, setSprintData] = useState<RoadmapPlan | null>(null);
  ...
  const totalCo2SavedInSprint =
    sprintData?.weeklyActions.reduce((acc, act, idx) => (completedDays[idx] ? acc + act.co2KgSaved : acc), 0) || 0;

  const totalMoneySavedInSprint =
    sprintData?.weeklyActions.reduce((acc, act, idx) => (completedDays[idx] ? acc + act.moneyEurSaved : acc), 0) || 0;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-6 space-y-8 animate-fadeIn">
      <div className="rounded-3xl bg-gradient-to-r from-emerald-600 via-teal-600 to-slate-900 text-white p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-md">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-emerald-200">{userProfile.district.split("(")[0]}</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
            {sprintData?.personalizedTagline ||
              (isFinnish ? `${userProfile.name} — Espoon 2030 Ilmastokumppani` : `${userProfile.name} — Espoo 2030 Climate Partner`)}
          </h2>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          >
            {isGenerating ? <RotateCw className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4 text-amber-300" />}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="flex items-baseline gap-2">
            <div className="text-3xl font-black text-slate-900">{userProfile.estimatedFootprintTonnes}</div>
          ...
          <div className="space-y-1.5 pt-2 border-t border-slate-100 text-xs">
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(100, (2.5 / userProfile.estimatedFootprintTonnes) * 100)}%` }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-2xl bg-emerald-50 border border-emerald-100 text-center">
              <div className="text-2xl font-black text-emerald-800">-{totalCo2SavedInSprint.toFixed(1)} kg</div>
            ...
            <div className="p-3 rounded-2xl bg-amber-50 border border-amber-100 text-center">
              <div className="text-2xl font-black text-amber-800">+{totalMoneySavedInSprint.toFixed(1)} €</div>
              <div className="text-[10px] text-amber-950 font-bold">Rahasäästö</div>
            </div>
          </div>

          <p className="text-[11px] text-slate-500 italic">
            {isFinnish ? "Rastita alta suorittamasi arjen toimenpiteet." : "Check off actions below as you complete them."}
          </p>
        </div>
            ...
            return (
              <div
                ...
                className={`p-5 rounded-2xl border transition cursor-pointer flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                  isDone ? "bg-emerald-50/60 border-emerald-300 ring-1 ring-emerald-400/20" : "bg-slate-50 border-slate-200 hover:bg-slate-100/70"
                }`}
              >
                <div className="flex items-start gap-3.5">
                  <div className="mt-0.5 shrink-0">
                    {isDone ? <CheckCircle2 className="w-5 h-5 text-emerald-600" /> : <Circle className="w-5 h-5 text-slate-300 hover:text-slate-400" />}
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-emerald-800">{action.category}</span>
                    </div>

                    <h4 className={`text-xs font-extrabold ${isDone ? "text-emerald-950 line-through opacity-80" : "text-slate-900"}`}>
                      {action.actionTitle}
                    </h4>

                    <p className="text-[11px] text-slate-600 leading-relaxed max-w-xl">{action.impactDescription}</p>
                ...
                <div className="flex sm:flex-col items-end gap-1 shrink-0 self-end sm:self-center">
                  ...
                  <span className="text-[10px] text-slate-500 font-bold">+{action.moneyEurSaved} € säästö</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="rounded-3xl bg-slate-900 text-white p-6 sm:p-8 space-y-4 shadow-sm">
        ...
        <div className="pt-2 flex flex-wrap gap-2 text-[11px]">
          <span className="px-3 py-1 rounded-lg bg-slate-800 text-emerald-300 border border-slate-700 font-medium">ARA Energia-avustus</span>
