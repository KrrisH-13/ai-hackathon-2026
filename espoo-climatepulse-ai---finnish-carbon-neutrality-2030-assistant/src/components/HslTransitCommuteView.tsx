...
import { CommuteComparison } from '../types/climate';
import { compareCommuteAPI } from '../services/aiClient';
...
export const HslTransitCommuteView: React.FC<HslTransitCommuteViewProps> = ({
  ...
  const [comparison, setComparison] = useState<CommuteComparison | null>({
    ...
    modes: [
      ...
      },
      {
        ...
        convenienceScore: 8,
        routeDetails: isFinnish
          ? 'Länsiväylää pitkin Otaniemen liittymään + pysäköinti.'
          : 'Via Länsiväylä to Otaniemi exit + campus parking.',
      },
      {
  ...
  const PRESET_ROUTES = [
    {
      from: 'Matinkylä (Iso Omena)',
      to: isFinnish ? 'Otaniemi (Aalto-yliopisto)' : 'Otaniemi (Aalto University)',
    },
    {
      from: 'Leppävaara (Sello)',
      to: 'Keilaniemi (Pikaratikka 15)',
    },
    {
      from: isFinnish ? 'Espoon keskus' : 'Espoo Centre',
      to: isFinnish ? 'Helsingin päärautatieasema (E/U-juna)' : 'Helsinki Central (E/U Train)',
    },
    {
      from: 'Tapiola',
      to: 'Ruoholahti / Kamppi (Länsimetro)',
    },
    {
      from: 'Kivenlahti',
      to: 'Tapiola (Länsimetro)',
    },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-6 space-y-8 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
          <div className="md:col-span-5 space-y-1">
            <label className="text-xs font-bold text-slate-700">
              {isFinnish ? 'Lähtöpaikka (Osoite / Kaupunginosa):' : 'Origin:'}
            </label>
            <input
              type="text"
              value={origin}
          ...
          <div className="md:col-span-5 space-y-1">
            <label className="text-xs font-bold text-slate-700">
              {isFinnish ? 'Määränpää:' : 'Destination:'}
            </label>
            <input
              type="text"
              value={destination}
            ...
            <div className="flex items-center gap-4 shrink-0">
              <div className="p-3.5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-center min-w-[110px]">
                <div className="text-2xl font-black text-emerald-300">
                  -{comparison.yearlySavingIfSwitchingToTransit.co2Kg} kg
                </div>
              ...
              <div className="p-3.5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-center min-w-[110px]">
                <div className="text-2xl font-black text-amber-300">
                  +{comparison.yearlySavingIfSwitchingToTransit.moneyEur} €
                </div>
                <div className="text-[10px] text-slate-200 font-bold">
                  {isFinnish ? 'Rahasäästö' : 'Money Saved'}
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-center min-w-[110px] hidden sm:block">
                <div className="text-2xl font-black text-white">
                  🌲 {comparison.yearlySavingIfSwitchingToTransit.treesEquivalent}
                </div>
                <div className="text-[10px] text-slate-200 font-bold">
                  {isFinnish ? 'Puuta vastaava' : 'Trees equivalent'}
                </div>
              </div>
            </div>
          </div>

          {/* Mode Breakdown Cards Grid */}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`font-black ${
                          isZeroEmission ? 'text-emerald-600' : 'text-slate-900'
                        }`}
                      >
                        {m.co2Grams} g CO2
                      </span>
      ...
      {/* Winter Cycling & Baana Network Tips */}
